...
const shoppingCart = {
  ...
  methods: {
    ...
    purchase: () => { // purchase significa compra todo lo que hay en el carrito de compras
...
function renderStore() {
  const html = db.items.map(item => {
    ...
    <div class="item">
        ...
        <div class="actions">
        <button class="add" data-id="${item.id}">Add to the shopping cart</button>
        </div>
    </div>`;
  });
  ...
  const html = db.items.map(item => {
    ...
    return `
      <div class="item">
        ...
        <div class="qty">${item.qty} units</div>
        <div class="subtotal">
          Subtotal:${numberToCurrency(item.qty * dbItem.price)} units
        </div>
        <div class="actions">
...
`
  ...
  const bPuchase = document.querySelector("#bPurchase")
  if (bPuchase) {
    bPuchase.addEventListener("click", e => {
...
function numberToCurrency(n) { // el siguiente es una api para poder dar formato, para transformar un numero en una moneda 
  return new Intl.NumberFormat("en-US", {
    maximumSignificantDigits: 2,
    style: "currency",
    currency: "USD"
  }).format(n)  // esto significa que quiero que le de el formato a mi variable n
}